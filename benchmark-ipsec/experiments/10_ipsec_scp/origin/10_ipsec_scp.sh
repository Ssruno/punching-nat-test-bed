...
do
    ...
    echo "----------------------"
    echo "seconds;size;filename;iteration" > /vagrant/experiments/10_ipsec_scp/results/10_result_${size_of_files[$file]}.csv
    for iteration in {1..30}
    do        
        # echo " $iteration"
        { /usr/bin/time  -f "%e; ${size_of_files[$file]}; ${name_of_files[$file]}; $iteration" scp -q /vagrant/experiments/10_ipsec_scp/origin/${name_of_files[$file]} vagrant@$destination:/vagrant/experiments/10_ipsec_scp/destination/ ; } 2>> /vagrant/experiments/10_ipsec_scp/results/10_result_${size_of_files[$file]}.csv
        sleep 1
    done
    echo "sleep 1 second"
    sleep 1
