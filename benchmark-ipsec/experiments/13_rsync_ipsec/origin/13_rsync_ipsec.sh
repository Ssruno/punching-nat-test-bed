...
name_of_files=(01_origin_file_0001M.dat 02_origin_file_0010M.dat 03_origin_file_0050M.dat)
...
do
    ...
    echo "----------------------"
    echo "seconds;size;filename;iteration" > /vagrant/experiments/13_rsync_ipsec/results/13_result_${size_of_files[$file]}.csv
    for iteration in {1..30}
    do        
        # echo " $iteration"
        { /usr/bin/time  -f "%e; ${size_of_files[$file]}; ${name_of_files[$file]}; $iteration" rsync -W /vagrant/experiments/13_rsync_ipsec/origin/${name_of_files[$file]} rsync://$destination:12000/files/ ; } 2>> /vagrant/experiments/13_rsync_ipsec/results/13_result_${size_of_files[$file]}.csv
        sleep 1
    done
    echo "sleep 1 second"
    sleep 1
