...
do
    ...
    echo "----------------------"
    echo "seconds;size;filename;iteration" > /vagrant/experiments/09_scp_plain/results/09_result_${size_of_files[$file]}.csv
    for iteration in {1..30}
    do        
        # echo " $iteration"
        { /usr/bin/time  -f "%e; ${size_of_files[$file]}; ${name_of_files[$file]}; $iteration" scp -q /vagrant/experiments/09_scp_plain/origin/${name_of_files[$file]} vagrant@$destination:/vagrant/experiments/09_scp_plain/destination/ ; } 2>> /vagrant/experiments/09_scp_plain/results/09_result_${size_of_files[$file]}.csv
        sleep 1
    done
    echo "sleep 1 second"
    sleep 1
