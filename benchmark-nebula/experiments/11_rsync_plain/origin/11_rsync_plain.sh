...
do
    ...
    echo "----------------------"
    echo "seconds;size;filename;iteration" > /vagrant/experiments/11_rsync_plain/results/11_result_${size_of_files[$file]}.csv
    for iteration in {1..30}
    do        
        # echo " $iteration"
        { /usr/bin/time  -f "%e; ${size_of_files[$file]}; ${name_of_files[$file]}; $iteration" rsync -W /vagrant/experiments/11_rsync_plain/origin/${name_of_files[$file]} rsync://$destination:12000/files/ ; } 2>> /vagrant/experiments/11_rsync_plain/results/11_result_${size_of_files[$file]}.csv
        sleep 1
    done
    echo "sleep 1 second"
    sleep 1
